...
(grouped) data frames, and will try coerce other objects if possible. Like
`summary()`, `skim()`'s method for data frames presents results for every
column; the statistics it provides depend on the class of the variable.
...
The format of the results are a similar data frame, with some additional 
attributes. Of course, we can't really separate that data frame by type, so
columns are somewhat sparse, with quite a few missing variables.  Statistics
...
Skimming the transpose of the matrix will give rowwise results.
...
type. Unlike the original data the parititioned data only has columns
corresponding to the skimming functions use for this data type.
...
it like `dplyr::filter` on column types in the original data. Again, unsuitable
columns are dropped.
...
results; it always keeps the metadata column.
...
To add a statistic, create an `sfl()` (a `skimr` function list) for each class
...
expecially when you want lots of changes. To stop appending, set
`append = FALSE`.
...
histogram) on Windows. One common fix is to switch your locale. The
