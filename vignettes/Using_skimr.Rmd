...
* For `pander()` the asis option is not officially supported although 
it may work in some cases. To prevent using asis by default use `panderOptons()` to
set it to false.
...
For example, the open font "DejaVu Sans" from the `extra font` package supports these.
Please read the `extra font` documentation for the details of how to install a specific
font. Installation generally uses the procedure shown below.

```
library(extrafont)
fonts() # Lists installed fonts
# If desired font is not listed
font_install("DejaVu Sans")
```

To us a font in a markdown document you will want to modify the YAML in the file
