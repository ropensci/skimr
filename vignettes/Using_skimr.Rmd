...
attributes and two metadata columns: 
...
Most `dplyr` verbs should work as expected, but note that you need to use a
`skim_type` prefix to refer to correct column.
...
iris %>%
  skim() %>%
  focus(numeric.missing, numeric.mean)
```
