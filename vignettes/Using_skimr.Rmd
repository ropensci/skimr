...
package  includes support for data frames, grouped data frames and vectors 
(for a number of standard types of data). Like summary
...
The main focus of skimr is on data frames. This is by design since it relies 
extensively on the tidyverse vocabulary and is pipeable and those methods 
are designed for data frames. 

Unlike summary.data.frame(), the printed results of skim (those displayed in
...
The skimr package does not include a skim.matrix function in order to
preserve the ability to handle matrices in flexible ways (in contrast to 
`summary.matrix()`).   

Three possible ways to handle matrices with `skim` parallel the three 
variations of the mean function for matrices.
...
The `skim_to_list()` function returns of a list of the wide data frames for 
...
# The skim_with and skim_with_defaults functions
...
statistics remains in place until they are reset to the defaults._

The `skim_with_defaults()` function resets the list to the defaults. 

By default `skim_with()` appends the new statistics, but setting `append = FALSE` 
